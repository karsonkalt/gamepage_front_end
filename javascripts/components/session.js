class Session {
    static addLogoutButton() {
        ...
        `
        a.addEventListener("click", SessionAPI.clickLogout)
        const parent = header()
    ...
    static addLoginDiv() {
        ...
        parent.prepend(div)
        loginButton().addEventListener("click", SessionAPI.clickLogin)
        loginField().addEventListener("keypress", this.addLoginWithEnter)
    }

    static addLoginWithEnter(event) {
        if (event.key === "Enter") {
            SessionAPI.clickLogin()
        }
    }

    static addOpponentLoginWithEnter(event) {
        if (event.key === "Enter") {
            clickOpponentLogin()
        }
    }

    static addErrorMessage(errorObj) {
    ...
    static displayWelcomeMessage(username) {
    ...
    static addOpponentLoginDiv() {
        ...
        const parent = playSpace()
        parent.prepend(div)
        loginButton().addEventListener("click", SessionAPI.clickOpponentLogin)
        loginField().addEventListener("keypress", this.addOpponentLoginWithEnter)
    }
